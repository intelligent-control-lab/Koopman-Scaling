...
class LearnedCovWeight(nn.Module):
    """
    Tiny module that holds a log-parameter so we can map it to a positive scalar.
    """
    def __init__(self, init_val=1e-3):
        super().__init__()
        # Initialize with log(init_val) so exp(log_cov_weight) = init_val initially.
        self.log_cov_weight = nn.Parameter(torch.log(torch.tensor(init_val, dtype=torch.float64)))

    def forward(self):
        # Exponentiate to ensure positivity
        return torch.exp(self.log_cov_weight)