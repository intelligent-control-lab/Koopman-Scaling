...
# -------------------------------
# Training Function with modifications for Go2 data
# -------------------------------
def train(env_name, train_steps=20000, all_loss=0, encode_dim=12, layer_depth=5, c_loss=1, 
    ...
    print("Loading Go2 dataset...")
    dataset_path = "../Data/datasets/2025-03-13_08-18-11_trajnum59950_trajlen100_decimation4.npz"
    if not os.path.exists(dataset_path):
        raise FileNotFoundError("Go2 dataset file not found: " + dataset_path)
    data_npz = np.load(dataset_path)

    state_data = data_npz['state_data']
    action_data = data_npz['action_data']
    num_traj = state_data.shape[1]
    T = state_data.shape[0]  # 100

    combined_data = np.empty((T, num_traj, 12 + 37), dtype=state_data.dtype)
    for t in range(T-1):
        combined_data[t, :, :] = np.concatenate([action_data[t], state_data[t]], axis=-1)

    combined_data[T-1, :, :] = np.concatenate([np.zeros((num_traj, 12), dtype=state_data.dtype), state_data[T-1]], axis=-1)

    train_traj = int(num_traj * 0.8)
    Ktrain_data = combined_data[:, :train_traj, :]
    Ktest_data = combined_data[:, train_traj:, :]
    udim = 12
    ...
    in_dim = Ktest_data.shape[-1] - udim
    Nstate = in_dim
    ...
    net = Network(layers, Nkoopman, udim)
    eval_step = 100
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    net.to(device)
    net.double()
    ...
    while step < train_steps:
        for batch in train_loader:
            ...
            if all_loss:
                ...
                # loss = Kloss + 10 * Ploss + factor * Closs if c_loss else Kloss + 10 * Ploss
            else:
