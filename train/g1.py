...
# -------------------------------
# Training Function with modifications for G1 data
# -------------------------------
def train(env_name, train_steps=20000, all_loss=0, encode_dim=12, layer_depth=5, c_loss=1, 
    ...
    dataset_path = "../Data/datasets/2025-03-19_17-35-32_trajnum60000_trajlen100_decimation4.npz"
    if not os.path.exists(dataset_path):
        raise FileNotFoundError("G1 dataset file not found: " + dataset_path)
    ...
    combined_data = np.empty((T, num_traj, 87 + 37), dtype=state_data.dtype)
    for t in range(T-1):
        combined_data[t, :, :] = np.concatenate([action_data[t], state_data[t]], axis=-1)

    combined_data[T-1, :, :] = np.concatenate([np.zeros((num_traj, 37), dtype=state_data.dtype), state_data[T-1]], axis=-1)
